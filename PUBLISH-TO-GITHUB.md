# Publish Skyrim Forge 4.2.0

Extract the GitHub-ready ZIP. Push the contents inside `Skyrim-Forge-4.2.0` to the repository root, not the outer ZIP.

```bash
git init
git add .
git commit -m "Release Skyrim Forge 4.2.0"
git branch -M main
...
```bash
git tag -a v4.2.0 -m "Skyrim Forge 4.2.0"
